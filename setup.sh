...
kubectl apply --filename crossplane-dependencies
...
kubectl apply --filename dot-application.yaml
...
yq --inplace \
    ...
    komoplane-values.yaml

yq --inplace \
    ".ingress.hosts[0].host = \"komoplane.$INGRESS_HOST.nip.io\"" \
    komoplane-values.yaml

helm upgrade --install komoplane komoplane \
    --namespace komodor --create-namespace \
    --values komoplane-values.yaml \
    --repo https://helm-charts.komodor.io --wait
